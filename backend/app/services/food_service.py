...
class FoodService:
    ...
    async def recognize_food_vit(self, image_base64: str) -> dict:
        ...
        # Load ViT model once per process (module-level singleton)
        ...
        # Classify
        try:
            inputs = _vit_extractor(images=img, return_tensors="pt")
            with torch.no_grad():
        ...
        for i in range(len(top5.indices)):
            idx = top5.indices[i].item()
            label = _vit_model.config.id2label[idx]
        ...
        if top_food and settings.API_NINJAS_KEY:
            nutrition = await self._fetch_api_ninjas_nutrition(top_food)
    ...
    async def _fetch_api_ninjas_nutrition(self, food_name: str) -> dict | None:
