...
def _try_load_local_llm():
    ...
    try:
        ...
        _local_llm.load_state_dict(torch.load(model_path, map_location=_local_device))
        _local_llm.eval()
...
class CoachService:
    ...
    def chat(
        ...
        try:
            ...
            if settings.OPENROUTER_API_KEY:
                try:
                    llm_response = self._generate_llm_response(message, user_context)
                    if llm_response:
    ...
    def _generate_llm_response(self, message: str, context: dict) -> Optional[str]:
        """Try local CoachLLM first, then fall back to OpenRouter API."""
        ...
        if _local_llm and _local_tokenizer:
            try:
                ...
                if "coach responds:" in text.lower():
                    response = text.lower().split("coach responds:")[-1].strip()
                    if len(response) > 10:
                        return response
            except Exception:
                pass
        ...
        if settings.OPENROUTER_API_KEY:
            try:
                ...
                stats = context.get("stats")
                system_prompt = (
                    "You are TrackFit AI Coach, a friendly fitness and nutrition assistant specializing in Indian food.\n"
                    f"User goal: {profile.goal if profile else 'general'}\n"
                    f"Weight: {profile.weight_kg if profile else 'unknown'}kg\n"
                    f"Today: Calories={stats.calories_consumed if stats else 0}, "
                    f"Water={stats.water_ml if stats else 0}ml, "
                    f"Steps={stats.steps if stats else 0}\n"
                    "Give concise, actionable advice. Use bullet points. Under 200 words. "
                    "Reference Indian foods when relevant."
                )
                resp = httpx.post(
