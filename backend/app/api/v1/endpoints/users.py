...
def get_me(current_user: Optional[User] = Depends(get_optional_user)):
    """Return current user info. Returns 404 for guests instead of 401."""
...
def create_profile(
    ...
    if current_user is None:
        raise HTTPException(status_code=401, detail="Login required to create profile")
...
def update_profile(
    ...
    if current_user is None:
        raise HTTPException(status_code=401, detail="Login required to update profile")
    profile = db.query(Profile).filter(Profile.user_id == current_user.id).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Profile not found")
    ...
    if recalc_fields & set(update_data.keys()):
        from app.schemas.profile import ProfileCreate as PC
        profile_data = PC(
            goal=profile.goal,
